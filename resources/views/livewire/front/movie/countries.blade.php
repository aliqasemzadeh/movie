        ...
        <div class="space-y-4">
            ...
            <ul class="divide-y divide-gray-200">
                @forelse($items as $item)
                    <li>
                        <a href="{{ route('front.movie.country', ['slug' => $item->slug]) }}" class="flex items-center justify-between py-2 hover:text-blue-600">
                            <span>{{ $item->name }}</span>
                ...
                @empty
                    <li class="py-6 text-center text-sm text-gray-500">{{ __('No countries found') }}</li>
                @endforelse
            </ul>
        </div>
