<x-livewire-modal::stack>
    <x-livewire-modal::modal position="center" class="w-full max-w-md overflow-auto rounded-lg bg-white dark:bg-gray-900 dark:text-gray-100 p-5">
        <div class="space-y-4">
            <h2 class="text-lg font-semibold">{{ __('quickpanel.country') }}</h2>
            <input type="search" placeholder="{{ __('Search countries...') }}" wire:model.debounce.300ms="search"
            ...
            <ul class="divide-y divide-gray-200 dark:divide-gray-700">
                @forelse($items as $item)
                    <li>
                        <a href="{{ route('front.movie.country', ['slug' => $item->slug]) }}" class="flex items-center justify-between py-2 hover:text-blue-600 dark:hover:text-blue-400">
                            <span>{{ $item->name }}</span>
                ...
                @empty
                    <li class="py-6 text-center text-sm text-gray-500 dark:text-gray-400">{{ __('No countries found') }}</li>
                @endforelse
            </ul>
        </div>
